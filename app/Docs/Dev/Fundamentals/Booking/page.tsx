        ...
        <>
            ...
            <article className="mt-4 w-full min-w-0 max-w-6x1 px-1 md:px-6 min-height-[calc(-103px + 100vh)]">
                <div className="-mt-4 mb-7 md:mb-10 md:mt-2">
                    ...
                    <p>{`In the above code snip at line 37, it can be seen that the street ID is checked to ensure that it's not equal to negative two during the object conversion process. -2 is set for failed results rather than -1 since -1 in the dispatch server is specifically used in the place of leaving a field absent, so -2 has been used so that ADI can distinguish if an ID has returned an erroneous result, whilst still allowing fields to be left empty for other operations. This is particularly important when attempting to submit a forced booking, because forced bookings street ID fields need to be set to -1 for a successful booking submission.`}</p>
